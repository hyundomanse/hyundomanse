...
public class StringUtil {
     ...
     * @param object 원본 객체
     * @return resultVal float
     */
    public static float isNullToFloat(Object object) {
        String string = "";
        float fl = 0;
        if (object != null) {
            string = object.toString().trim();
            fl =   Float.valueOf(string).floatValue();
        }
        return fl;
    }
    ...
    /**
     * 문자가 null인지 확인하고 null인 경우 true 로 return
     * @param String 원본 문자열
     * @return true/false
     */
    ...
    /**
     * 객체가 null인지 확인하고 null인 경우 true 로 return
     * @param object 원본 객체
     * @return true/false
     */
...
/**
	...
	public static Object instantiate(String className) throws ClassNotFoundException, InstantiationException, IllegalAccessException, Exception {
	 ...
	 /**
	  * 문자열이 지정한 길이가 넘어가면 해당길이로 짤라주고 짧으면 지정한 문자를 지정길이만큼 더해준다.  2014/06/02 추가
	  * @param input 원본 문자열
	  * @param output 더해줄문자열  
	  * @param slength 지정길이
	  * @param Type E:뒤에 붙임, S:앞에붙임  
	  * @return 지정길이로 잘르거나 더할분자열 합친 문자열
	  */
	 public static String makeString(String input, String output, int slength, String type) {
			
			int leng = slength-input.length();
	        String returnVal = null;
	        if (input != null) {
	            if (input.length() < slength) {
					...
					returnVal = input.substring(0, slength);
				}
	 	...
	 	/**
	     * 문자열에   slength만큼 뒤에 소수점을 붙여준다 2014/06/09 추가
	     * @param input 원본 문자열
	     * @param slength 지정길이
	     * @return slength만큼 뒤에 소수점을 붙인 문자열
	     */
		public static String makeNumber(String input, int slength) {
	        ...
	        if(input.indexOf(".") == -1){ //소수점이 없을때
	        	...
	        	if(input.length() - slength == input.indexOf(".") + 1){
	        		returnVal = input;
	        	}else if(input.length() - slength < input.indexOf(".") + 1){
	        		int rowCnt = ((input.indexOf(".") + 1) - (input.length() - slength));
	        		for (int i = 0; i < rowCnt; i++) {
	        			input = input + "0";	
					}
	        		returnVal= input;
	        	}else{
	        		returnVal = input.substring(0, input.indexOf(".") + 1 + slength);
	        	}
		...
		*/
		public static String makeMoneyType(String no){
			...
			if(index == -1){
				...
				return (
					makeMoneyType( Long.parseLong(no.substring(0, index)) )+no.substring(index, no.length())
				);
			}
		}		
		...
		/**
		* 지정한문자열로 왼쪽부터 채우는 함수.
		* @param  String value, int padLen, char padChar
		* @return String value
		*/		
		public static String lpad(String value, int padLen, String padChar){
